...
def assert_deleted_item_ids_in_response(response: Response, ids_to_check: list[dict]) -> None:
...
def assert_added_product_not_in_api_response(response: Response, added_products: list[dict]) -> None:
    """
    ...
    response_product_ids = [item['productInfo']['id'] for item in response_data['items']]
