...
class TestAuthentication:
    ...
    def test_registration(self, postgres):
        ...
        with step("Verify JWT token is returned after confirmation and content type"):
            ...
            assert_content_type(response_after_confirmation, expected_content_type=expected_content_type)

        with step("Verify that user is successfully registered by getting info about user"):
            UsersAPI().get_user(
                token=response_after_confirmation.json()["token"]
            )
        ...
        with step("Getting info about the user in DB"):
            user_data = postgres.get_data_by_filter(
                table="user_details", field="email", value=data_for_registration['email']
            )
