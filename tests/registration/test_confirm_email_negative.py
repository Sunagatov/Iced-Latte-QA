...
class TestConfirmationEmail:
    ...
    def test_confirmation_email_for_registration_with_empty_and_invalid_format_code(
        ...
        with step("Confirm email for registration with empty code"):
            response_confirmation = AuthenticateAPI().confirmation_email(
                code=code, expected_status_code=400
            )

            assert_message_in_response(
                response=response_confirmation, expected_message=expected_message_part
            )
    ...
    def test_confirmation_email_with_used_code(
        ...
        with step("Confirm email for registration with already used code"):
            ...
            response_confirmation = AuthenticateAPI().confirmation_email(
                code=code_already_used, expected_status_code=400
            )
            expected_message = "Incorrect token"
            assert_response_message(
                response=response_confirmation, expected_message=expected_message
            )
