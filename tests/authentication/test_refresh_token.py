...
class TestLogout:
    ...
    def test_refresh_access_token_with_valid_refresh_token(
        ...
        with step("Registration of user"):
            user, access_token, token_sent_for_refresh = (
        ...
        with step("Verify access token and refresh token in response"):
            ...
            assert_that(
                ...
                is_not(empty()),
                reason="Token is not in response",
            )
            assert_that(
        ...
        with step(
            "Verify that access token and refresh token are valid after refresh by getting user's info"
        ):
            UsersAPI().get_user(
                token=access_token_after_refresh, expected_status_code=200
            )
            UsersAPI().get_user(
                token=refresh_token_after_refresh, expected_status_code=200
            )
