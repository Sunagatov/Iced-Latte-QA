...
class TestForgotPassword:
    ...
    @pytest.mark.xfail(
        reason="HTTP CODE - 400 but got 401. Error message not correct according to requirement"
    )
    @pytest.mark.critical
    @severity(severity_level="MAJOR")
    @title(
        "Test forgot password, sent request to reset password with invalid/empty email"
    )
    @description(
        "Give user registered."
    ...
    @pytest.mark.parametrize(
        "email_to_reset_password, expected_message",
        [
    ...
    def test_forgot_password_email_not_valid(
        self, email_to_reset_password, expected_message
    ):
        with step("Sent request to reset password with invalid email"):
            response_to_reset_password = AuthenticateAPI().forgot_password(
                email=email_to_reset_password, expected_status_code=400
            )
            assert_response_message(
                response=response_to_reset_password, expected_message=expected_message
            )
    ...
    def test_forgot_password_email_not_exist(self):
        with step("Sent request to reset password with not exist email in BD"):
            ...
            response_to_reset_password = AuthenticateAPI().forgot_password(
                email=email_not_exist, expected_status_code=401
            )
