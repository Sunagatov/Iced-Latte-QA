...
class TestChangePassword:
    ...
    @title(
        "Test change user's password with less than 8 characters and more than 128 characters"
    )
    @description(
    ...
    def test_change_user_password_with_incorrect_length(
        self, create_authorized_user, new_password, expected_message
    ):
        ...
        with step("Change user password"):
            response_changing_password = UsersAPI().change_password(
                token=token,
        ...
        with step(
            "Verify that password not successfully changes by"
            " sent request for Authentication with new password "
        ):
            AuthenticateAPI().authentication(
                email=user["email"], password=new_password, expected_status_code=401
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                generate_numeric_password(length=8),
                "Password must be at least 8 characters long and contain at least one letter, one digit, and may include special characters @$!%*?&",
            ),
            pytest.param(
                generate_numeric_password(length=9),
                "Password must be at least 8 characters long and contain at least one letter, one digit, and may include special characters @$!%*?&",
            ),
            pytest.param(
    ...
    def test_change_user_password_with_incorrect_format(
        self, create_authorized_user, new_password, expected_message
    ):
        with step("Registration of user"):
            user, token = (
    ...
    @pytest.mark.parametrize(
        "new_password, old_password, expected_message",
        [
    ...
    def test_change_user_password_with_empty_fields(
        self, create_authorized_user, new_password, old_password, expected_message
    ):
        with step("Registration of user"):
            user, token = (
                create_authorized_user["user"],
        ...
        with step("Change user password"):
            new_password = (
                generate_password(8) if new_password is None else new_password
            )
    ...
    def test_change_user_password_with_incorrect_old_password(
        ...
        with step("Verify response message"):
            email = user["email"]
            expected_message = (
                f"User with userEmail = '{email}' provided incorrect password."
            )
            assert_response_message(
                response=response_change_password, expected_message=expected_message
            )
    ...
    def test_change_user_password_with_token_not_containing_email(
        self, create_authorized_user
    ):
        with step("Registration of user"):
            user, token = (
    ...
    def test_change_user_password_with_token_containing_not_exist_user_email(
        self, create_authorized_user
    ):
        with step("Registration of user"):
            user, token = (
