...
class TestChangePassword:
    ...
    @pytest.mark.parametrize("new_password",
                             [generate_password(8), generate_password(9), generate_password(63), generate_password(64),
                              generate_password(65), generate_password(127), generate_password(128)])
    def test_change_user_password_with_valid_length(self, create_authorized_user, new_password):
        ...
        with step("Verify that password successfully changes by"
                  " sent request for Authentication with new password "):
            response_auth = AuthenticateAPI().authentication(email=user["email"], password=new_password,
                                                             expected_status_code=200)
    ...
    @pytest.mark.parametrize("new_password", [pytest.param(generate_string(4) + generate_numeric_password(length=4)),
    ...
    def test_change_user_password_with_correct_format(self, create_authorized_user, new_password):
        with step("Registration of user"):
            user, token = create_authorized_user["user"], create_authorized_user["token"]
        ...
        with step("Verify that password  successfully changes by"
                  " sent request for Authentication with new password "):
            AuthenticateAPI().authentication(email=user["email"], password=new_password, expected_status_code=200)
