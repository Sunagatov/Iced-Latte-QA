...
class TestCart:
    ...
    def test_deleting_items_from_cart(
        ...
        with step("Generating data for delete"):
            ...
            response_delete_item = CartAPI().delete_item_from_cart(
                token=token, cart_item_id=item_to_delete_more_than_one
            )
        ...
        with step("Verify that deleted items do not exist in shopping cart"):
            assert_deleted_item_ids_in_response(
                response_delete_item, item_to_delete_more_than_one
            )
    ...
    def test_deleting_one_item_from_cart(
        self, creating_and_adding_product_to_shopping_cart
    ):
        with step(
            "Registration new user, and add product to the shopping cart"
        ...
        with step("Generating data for delete one product from cart"):
            id_one_item_to_delete = [
                response_get_cart_after_added.json()["items"][0]["id"]
            ]

        with step("Deleting item from cart"):
            response_delete_item = CartAPI().delete_item_from_cart(
                token=token, cart_item_id=id_one_item_to_delete
            )
            assert_content_type(response_delete_item, "application/json")

        with step("Verify that deleted items do not exist in shopping cart"):
            assert_deleted_item_ids_in_response(
                response_delete_item, id_one_item_to_delete
            )
