...
from data.data_for_cart import data_for_not_exist_shopping_cart_item_id
...
class TestCart:
    ...
    def test_updating_decrease_item_quantity_in_cart(
        self, creating_and_adding_product_to_shopping_cart
    ):
        with step(
            "Registration new user, and add product to the shopping cart"
            "Getting info about user and user's shopping cart "
        ):
            (
                token,
                new_user_id,
                response_get_cart_after_added,
            ) = creating_and_adding_product_to_shopping_cart

        with step(
            "Getting random items to update from shopping cart and determination quantity for update "
        ):
            random_item_from_cart = extract_random_item_detail(
                response_get_cart_after_added
            )
        ...
        with step("Update quantity of item"):
            response_after_update = CartAPI().update_quantity_product(
                token=token,
        ...
        with step("Verify the error message from request for update"):
            ...
            assert_response_message(
                response_after_update, expected_message=expected_message
            )
        ...
        with step("Get shopping cart of user and verify that quantity is not updated"):
            ...
            actual_item_in_cart = get_quantity_specific_cart_item(
                response_get_cart, item_id_to_update
            )
    ...
    def test_updating_not_exist_item_in_cart(
        self, creating_and_adding_product_to_shopping_cart
    ):
        with step(
            "Registration new user, and add product to the shopping cart"
        ...
        with step("Update quantity of product"):
            ...
            response_after_update = CartAPI().update_quantity_product(
                token=token,
                item_id=data_for_update_not_exist_item["shoppingCartItemId"],
                item_quantity=data_for_update_not_exist_item["productQuantityChange"],
        ...
        with step("Checking the response body and the Content-Type"):
            ...
            assert_response_message(
                response_after_update, expected_message_after_update
            )
