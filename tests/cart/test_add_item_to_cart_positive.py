...
class TestCart:
    ...
    def test_add_item_to_cart(self, create_authorized_user):
        ...
        with step(
            "Get user's shopping cart and verify that user doesn't have items in shopping cart"
        ):
        ...
        with step("Add product to the shopping cart "):
            ...
            response_add_to_cart = CartAPI().add_item_to_cart(
                token=token, items=product_add_to_cart
            )

        with step("Verify product to add in response after API request"):
            product_list_in_response_add_to_cart = get_product_info(
                response=response_add_to_cart
            )
            assert_product_to_add_matches_response(
                product_add_to_cart, product_list_in_response_add_to_cart
            )

        with step(
            "Verify the shopping cart created under new user/response body contain correct user's id)."
        ):
            response_get_cart_after_add_product = CartAPI().get_user_cart(token=token)
            expected_user_id_in_cart = response_get_cart_after_add_product.json()[
                "userId"
            ]
            assert_that(
                new_user_id,
        ...
        with step("Verify added products are in a shopping cart"):
            product_list_in_cart_after_add = get_product_info(
                response=response_get_cart_after_add_product
            )
            assert_product_to_add_matches_response(
                product_add_to_cart, product_list_in_cart_after_add
            )
